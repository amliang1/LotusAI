...
grep -n "struct AIModelConfigRow: View {" LotusAI/Views/AIModelConfigView.swift | while read -r line; do
...
grep -n "struct NewAIModelConfigView: View {" LotusAI/Views/AIModelConfigView.swift | while read -r line; do
  line_num=$(echo "$line" | cut -d':' -f1)
  if [ ! -z "$line_num" ]; then
    # Find the matching closing brace
    end_line=$(tail -n +$line_num LotusAI/Views/AIModelConfigView.swift | grep -n "^}" | head -1 | cut -d':' -f1)
